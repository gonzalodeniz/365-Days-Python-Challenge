...
def test_inicializacion_matriz() -> None:
    m = Matriz(3, 3, 0)
...
def test_matriz_de_listas() -> None:
...
def test_contador_submatrices() -> None:
...
def test_validacion() -> None:
    with pytest.raises(AssertionError):
        Matriz('a', 3)                      # type: ignore
    with pytest.raises(AssertionError):
        Matriz(3, 'b')                      # type: ignore
    with pytest.raises(AssertionError):
        Matriz.de_listas([1, 2, 3])         # type: ignore
    m = Matriz(3, 3, 0)
    with pytest.raises(AssertionError):
        m.get(3, 3)
    with pytest.raises(AssertionError):
